...
import type { Playable, DisplayArea } from '../../Plugin.d.ts'
...
function App() {
  ...
  // - pluginScreen: 全画面。
  const wrapperClass = 
    isOverlay ? "fixed bottom-0 left-0 w-full p-6 flex justify-start items-end pointer-events-none" :
    area.type === 'pluginSettings' ? "flex items-center justify-center min-h-screen p-4" :
    "w-screen h-screen p-0";

  return (
    <div className={`${wrapperClass} bg-transparent transition-all duration-500`}>
      <div 
        style={{ 
      ...
      >
        ...
        <div className="p-4 flex-1 overflow-auto scrollbar-hide">
          <h2 className={`m-0 mb-1 font-bold leading-tight ${isOverlay ? 'text-base' : 'text-xl'}`}>
            {playable.title}
          </h2>
        ...
        <div className="px-3 py-1.5 bg-white/5 border-t border-white/10 text-[9px] text-gray-500 flex justify-between items-center shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="opacity-40">{area.width} × {area.height}</span>
          </div>
          {isOverlay && <span className="text-red-500 animate-pulse font-bold text-[8px] flex items-center gap-1">
